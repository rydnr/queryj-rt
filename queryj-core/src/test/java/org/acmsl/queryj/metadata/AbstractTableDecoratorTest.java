...
public class AbstractTableDecoratorTest
{
    ...
    public void getAll_includes_parent_attributes()
    {
        ...
        @NotNull final ListDecorator<Attribute<DecoratedString>> allAttributes = instance.getAllAttributes();

        Assert.assertEquals(2, allAttributes.size());

        Assert.assertTrue(allAttributes.getItems()childAttribute));
        Assert.assertTrue(allAttributes.contains(parentAttribute));
    }
}
