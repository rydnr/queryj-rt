...
public class CustomSqlValidationHandlerTest
{
        ...
        throws Exception
    {
        ...
        @NotNull final Table<String, Attribute<String>, List<Attribute<String>>>
            t_Table = PowerMock.createNiceMock(Table.class);
        @NotNull final List<Sql<String>> t_lSql = Arrays.asList(sql);

        EasyMock.expect(t_MetadataManager.getTableDAO()).andReturn(t_TableDAO);
        EasyMock.expect(t_TableDAO.findByName(sql.getDao())).andReturn(t_Table);
        EasyMock.expect(t_Table.getAttributes()).andReturn(new ArrayList<Attribute<String>>(0));
